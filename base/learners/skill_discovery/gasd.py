...
class GASDSACV2Learner(BaseSACV2Learner, BaseEDLLearner):
    ...
    Final robust argument filtering.
    """
    AGENT_TYPE = 'GASD'

    def __init__(self, **kwargs):
        # 1. Capture and REMOVE all GASD specific arguments from kwargs
        # This prevents parent classes from throwing TypeErrors
        ...
        # Ensure BaseLearner init logic is satisfied
        ...
        # 3. Attributes fix-up after super call
        self.im_nu = self.im_nu_val
        self.im_lambda = 0.0
        
        if "rew_pot" not in self.ep_summary_keys:
            self.ep_summary_keys += ["rew_pot", "rew_pen", "rew_gauss", "rew_bon"]
            
        print("GASD Learner initialized successfully.")

    def _make_im_modules(self):
        """Called by BaseLearner. Returns the already created spectra engine."""
        # Use attributes saved on self
    ...
    def _make_agent_modules(self):
        ...
        kwargs = dict(env=self._dummy_env, hidden_size=self.hidden_size, num_layers=self.num_layers,
                      goal_size=skill_n, normalize_inputs=self.normalize_inputs)
    ...
    def fill_summary(self, *values):
        ...
        extra_vals = [torch.tensor(b.get('potential', 0.0)), torch.tensor(b.get('penalty', 0.0)),
                      torch.tensor(b.get('gauss', 0.0)), torch.tensor(b.get('bonus', 0.0))]
        super(GASDSACV2Learner, self).fill_summary(*(values + tuple(extra_vals)))
